...
function cacheTahfizhDOM() {
    const elementMapping = {
        ...
        mutqinUnggulanProgressContainer: 'tahfizh-mutqin-unggulan-progress-container',
        mutqinUnggulanCircle: 'tahfizh-mutqin-unggulan-circle',
        mutqinJuz30Circle: 'tahfizh-mutqin-juz30-circle',
        mutqinJuz29Circle: 'tahfizh-mutqin-juz29-circle',
        mutqinUnggulanDetails: 'tahfizh-mutqin-unggulan-details',
...
function processTahfizhResponse(response) {
    ...
    // 2. Simpan program siswa asli dari spreadsheet Tahfizh untuk mapping
    if (response.santri) {
        TahfizhState.studentProgramMap = new Map(response.santri.map(s => [
            normalizeTahfizhName(s.Nama || s.NamaSantri || s.nama),
            s.Program || s.program
        ]));
    }
...
function calculateTahfizhSantriStats() {
    ...
    santriStatsMap.forEach(santri => {
        ...
        const mutqinSetengahJuz = validSetoran.find(s => s.jenis === 'Mutqin' && String(s.juz) === "juz30_setengah" && new Date(s.createdAt) <= TahfizhConfig.deadlineJuz30Score);
        const mutqinJuz30 = validSetoran.find(s => s.jenis === 'Mutqin' && s.juz == 30);
        const mutqinJuz29 = validSetoran.find(s => s.jenis === 'Mutqin' && s.juz == 29);

        if (mutqinJuz29) santri.mutqinJuz.add(29);
        if (mutqinJuz30) santri.mutqinJuz.add(30);

        // Hitung nilai/skor kelulusan
        if (mutqinSetengahJuz || (mutqinJuz30 && new Date(mutqinJuz30.createdAt) <= TahfizhConfig.deadlineJuz30Score)) {
            santri.nilai = 100;
        } else {
        ...
        } else if (santri.program === 'Tahfizh' && santri.nilai >= 100 && santri.mutqinJuz.has(29) && santri.mutqinJuz.has(30)) {
            santri.isTuntas = true;
        }
...
function checkTahfizhPerpulangan(santri) {
    ...
    // Lulus instan jika punya mutqin 30 atau setengah juz 30
    if (validSetoran.some(s => s.jenis === 'Mutqin' && (s.juz == 30 || String(s.juz) === 'juz30_setengah'))) {
        return 'Boleh Pulang';
    }
    ...
    if (TahfizhConfig.perpulanganPeriods) {
        for (const period of TahfizhConfig.perpulanganPeriods) {
            ...
            if (period.type === 'surat') {
                ...
                conditionMet = validSetoran.some(set => 
                    set.jenis === 'Mutqin' && period.required.includes(String(set.juz)) && new Date(set.createdAt) <= period.deadline
                );
            }
    ...
    return latestAchievedPeriod ? 'Boleh Pulang' : 'Belum Boleh Pulang';
}
...
function buildTahfizhClassGroups() {
    ...
    TahfizhState.classGroups['Khusus Tahfizh'] = {
        santri: TahfizhState.santriData.filter(s => s.program === 'Tahfizh'),
...
function renderTahfizhValidationInbox() {
    ...
    TahfizhState.pendingSetoran.forEach(s => {
        ...
        setElementText(clone, '.val-info', `${s.program} - Kelas ${s.kelas}`);
        
        const detailText = `${s.jenis} • ${s.juz === 'juz30_setengah' ? '1/2 Juz 30' : 'Juz ' + s.juz} • ${s.halaman ? s.halaman + ' hlm' : s.surat}`;
...
function renderTahfizhProgressCircles() {
    ...
    const unggulan = TahfizhState.santriData.filter(s => s.program === 'Unggulan');
    const tahfizh = TahfizhState.santriData.filter(s => s.program === 'Tahfizh');

    createCircle(TDOM.mutqinUnggulanProgressContainer, TDOM.mutqinUnggulanCircle, TDOM.mutqinUnggulanDetails, unggulan, 'text-sky-500', s => s.nilai >= 100);
    createCircle(TDOM.mutqinJuz30ProgressContainer, TDOM.mutqinJuz30Circle, TDOM.mutqinJuz30Details, tahfizh, 'text-green-500', s => s.mutqinJuz.has(30));
    createCircle(TDOM.mutqinJuz29ProgressContainer, TDOM.mutqinJuz29Circle, TDOM.mutqinJuz29Details, tahfizh, 'text-amber-500', s => s.mutqinJuz.has(29));
}
...
function renderTahfizhLeaderboard() {
    ...
    // Default load tab pertama (Tahfizh)
    renderTahfizhPeringkatContent('Tahfizh');
    const tab = TDOM.peringkatSection.querySelector(`[data-program="Tahfizh"]`);
    if (tab) {
        tab.className = "tab-peringkat active px-3 py-1.5 text-xs font-bold rounded-lg transition-all bg-white text-orange-600 shadow-sm border border-orange-100/30";
    }
}

function renderTahfizhPeringkatContent(program) {
    ...
    criteria.forEach(c => {
        ...
        if (topList.length > 0) {
            topList.forEach((s, i) => {
                ...
                const val = (s[c.key] || 0).toFixed(['ziyadahPages', 'unggulanPages'].includes(c.key) ? 1 : 0);
...
function renderTahfizhJuzTuntasTracking() {
    ...
    // Default load tab 30
    renderTahfizhJuzContent('30');
    const tab = TDOM.tahfizhTuntasTrackingSection.querySelector(`[data-target="30"]`);
    if (tab) {
...
function renderTahfizhJuzContent(targetKey, searchTerm = '') {
    ...
    setElementText(clone, '.text-progress-label', `${targetLabel}: ${tuntasCount} / ${totalCount} Santri`);
    ...
    if (listTuntasEl) listTuntasEl.innerHTML = tuntasList.map(s => `<li>${s.nama}</li>`).join('') || '<li class="text-slate-400 dark:text-slate-650 italic list-none">Tidak ada</li>';

    const listBelumEl = clone.querySelector('.list-belum');
    if (listBelumEl) listBelumEl.innerHTML = belumTuntasList.map(s => `<li>${s.nama}</li>`).join('') || '<li class="text-slate-450 dark:text-slate-650 italic list-none">Semua tuntas</li>';
...
function renderTahfizhHistoryTable() {
    ...
    const programFilter = TDOM.filterProgram.value;
    ...
    filtered.forEach(setoran => {
        ...
        setElementText(clone, '.data-juz-text', String(setoran.juz) === "juz30_setengah" ? "Setengah Juz 30" : `Juz ${setoran.juz}`);
        ...
        if (santri) {
            ...
            if (pIcon) {
                pIcon.innerHTML = santri.program === 'Unggulan' 
                    ? `<span title="Unggulan" class="inline-flex items-center justify-center h-5 w-5 rounded-lg bg-orange-100 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400 text-[10px] font-black border border-orange-200 dark:border-orange-900/30">U</span>` 
                    : `<span title="Tahfizh" class="inline-flex items-center justify-center h-5 w-5 rounded-lg bg-purple-100 dark:bg-purple-950/40 text-purple-600 dark:text-purple-400 text-[10px] font-black border border-purple-200 dark:border-purple-900/30">T</span>`;
            }
...
function renderSingleTahfizhRekapTable(groupName, isInitial = false) {
    ...
    group.santri.forEach((santri, idx) => {
        ...
        setElementText(row, '.col-ziyadah', santri.program === 'Tahfizh' ? (santri.ziyadahPages || 0).toFixed(1) : '-');
...
function exportTahfizhPDF(classGroup) {
    try {
        ...
        const body = group.santri.sort((a, b) => a.nama.localeCompare(b.nama)).map((s, idx) => [
            idx + 1, s.nama, s.nilaiTampil, s.program === 'Tahfizh' ? (s.ziyadahPages || 0).toFixed(1) : '-', s.isTuntas ? 'Tuntas' : 'Belum Tuntas'
        ]);
...
function openTahfizhDetailModal(santriId) {
    ...
    TDOM.detailInfo.textContent = `${s.kelas} - ${s.program} | Musyrif: ${s.musyrif}`;
...
function renderTahfizhSantriRaporDashboard(santriId) {
    ...
    setElementHTML(dash, '[data-program-text]', `<span>📖</span> ${s.program}`);
    ...
    if (perpulanganBadge) {
        ...
        perpulanganBadge.className = `text-xs font-bold px-3 py-1 rounded-md border shadow-sm ${s.statusPerpulangan === 'Boleh Pulang' ? 'bg-green-50 text-green-700 border-green-200 dark:bg-green-950/40 dark:text-green-400 dark:border-green-900/50' : 'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/40 dark:text-rose-400 dark:border-rose-900/50'}`;
    }

    setElementText(dash, '[data-nilai]', s.nilaiTampil);
    setElementText(dash, '[data-ziyadah]', s.program === 'Tahfizh' ? `${(s.ziyadahPages || 0).toFixed(1)}` : '-');
    setElementText(dash, '[data-total-setoran]', s.setoranCount);

    // Target wajib progres
    const targetWajibItems = [
        { label: 'Setengah Juz 30', met: s.nilai >= 100 },
        { label: 'Mutqin Juz 30', met: s.mutqinJuz.has(30), tahfizhOnly: true },
        { label: 'Mutqin Juz 29', met: s.mutqinJuz.has(29), tahfizhOnly: true }
    ];

    const targetsHtml = targetWajibItems.filter(item => !item.tahfizhOnly || s.program === 'Tahfizh').map(item => `
    ...
    if (s.program === 'Tahfizh' && s.isTuntas && ziyadahSect) {
        ziyadahSect.classList.remove('hidden');
        const ziyadahCont = dash.querySelector('[data-progres-ziyadah]');
        if (ziyadahCont) {
            const allJuz = Object.keys(TahfizhConfig.hafalanData?.juzPageCounts || {}).filter(j => !['juz30_setengah', '30', '29'].includes(j));
        ...
        tableBody.innerHTML = sortedActivities.length > 0 ? sortedActivities.map(act => `
            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                <td class="p-4 align-middle">
                    <p class="font-bold text-sm text-slate-800 dark:text-slate-200">${act.jenis} <span class="text-slate-400 mx-1">•</span> ${String(act.juz) === 'juz30_setengah' ? '1/2 Juz 30' : `Juz ${act.juz}`}</p>
...
function renderTahfizhRaporFocusJuz(santri) {
    ...
    const checkOrder = [...Array(30).keys()].map(i => i + 1).reverse(); // Juz 30 -> 1
...
function renderTahfizhRaporDynamicGrid(santri) {
    ...
    juzOrder.forEach(juzNum => {
        const setoranJuz = santri.setoran.filter(s => 
            (s.juz == juzNum || (juzNum === 30 && String(s.juz) === 'juz30_setengah')) && s.status === 'Verified'
        );
        ...
        setoranJuz.forEach(s => {
            if (s.jenis === 'Mutqin') {
                if (String(s.juz) === 'juz30_setengah') totalPagesDone += 9;
...
function handleFormSantriChange() {
    ...
    if (rawSantri) {
        ...
        TDOM.program.value = sStats?.program || 'Tahfizh';
        TDOM.santriId.value = sId;

        const isTuntas = sStats ? sStats.isTuntas : false;
        // Pilihan Jenis: Hanya santri Unggulan atau santri Tahfizh tuntas yang bisa ujian Mutqin/Muraja'ah
        const options = (sStats?.program === "Unggulan" || (sStats?.program === "Tahfizh" && isTuntas))
            ? ["Ziyadah", "Murajaah", "Mutqin"]
            : ["Ziyadah", "Murajaah", "Mutqin"]; // default bisa pilih semuanya
...
function handleFormJenisChange() {
    ...
    if (jenis && sStats && TahfizhConfig.hafalanData?.surahData) {
        if (jenis === 'Mutqin') {
            if (!sStats.nilai || sStats.nilai < 100) {
                juzOptions.push({ text: "Setengah Juz 30", value: "juz30_setengah" });
            }
            Object.keys(TahfizhConfig.hafalanData.surahData).forEach(j => {
                if (!sStats.mutqinJuz.has(parseInt(j))) {
                    juzOptions.push({ text: `Juz ${j}`, value: j });
                }
            });
        } else {
            Object.keys(TahfizhConfig.hafalanData.surahData).forEach(j => {
    ...
    populateTahfizhSelect(TDOM.juz, juzOptions.sort((a,b) => a.value - b.value), 'Pilih Juz');
...
function handleFormJuzChange() {
    ...
    if (!jenis || !juz || juz === 'juz30_setengah') return;
...
function handleTahfizhDelegatedClicks(e) {
    ...
    if (peringkatTab) {
        const program = peringkatTab.dataset.program;
        ...
        renderTahfizhPeringkatContent(program);
        return;
    }
